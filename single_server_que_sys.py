...
def accept_data2():
    ...
    service_time=float(input("How Much Time Schedule Customers Appointment time(min)(Int):"))
    simulet_time=float(input("How Much Time Simulet Customers in Your Systems(Hours)(Int):"))
    start_time=float(input(  "Enter The  Start Time Of Your    Systems      (Hours) (Int):"))
    #ran_n=int(input(       "How Many Random Number You Want in   Your      System (Int)"))
    ran_p=float((simulet_time*60)/service_time)
...
def next_step():
    ...
    service_time=float(v1.get())
    simulet_time=float(v2.get())
    start_time=float(v3.get())
    ran_p=int((simulet_time*60)/service_time)
...
def acept_val2():
    ...
    for i in range(n):
        time_req.append(float(entry1[i].get()))
        no_of_pat.append(float(entry2[i].get()))
